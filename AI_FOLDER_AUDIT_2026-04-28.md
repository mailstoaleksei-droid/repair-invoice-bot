...
- `Staack_Report`: active Staack downloader/report bot. GitHub repo created and current active code pushed: `mailstoaleksei-droid/staack-report`, commit `4d238f9`. Current Windows task `StaackBot` still points to `Groo Forest GmbH\Bot`; switching it to `AI\Staack_Report` was blocked by Windows run-as password policy, so run `update_staack_task_to_ai.ps1` manually from Administrator PowerShell.
