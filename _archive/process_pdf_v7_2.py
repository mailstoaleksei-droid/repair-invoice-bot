"""
PDF PROCESSOR v7.2 - ФИНАЛЬНАЯ ВЕРСИЯ
Unified Telegram + 16 поставщиков + правильная логика NETTO/BRUTTO
Исправлено: Auto Compass (Internal BRUTTO, External NETTO)
Добавлено: 13 новых функций извлечения + автоопределение года + защита от ошибок кодировки
"""
import os
import shutil
import pdfplumber
...
def get_processed_folder_by_year(date_str):
    """
    Определить папку для обработанных файлов по году из даты счёта
    Автоматически создаёт папки RG 2025, RG 2026, RG 2027 и т.д.
    """
    try:
        # Извлекаем год из даты (формат: DD.MM.YYYY)
        if '.' in date_str:
            parts = date_str.split('.')
            if len(parts) == 3:
                year = parts[2]
        ...
        # Базовый путь к папкам RG
        base_path = r"C:\Users\Aleksei Samosvat\Groo GmbH\Intranet - Groo GmbH - Dokumente\Auto Compass GmbH\AC - Fahrzeuge\Ablage\Eingangs Rechnungen"
        
        # Формируем путь к папке года
        year_folder = os.path.join(base_path, f"RG {year} Ersatyteile RepRG")
        ...
        print(f"  ⚠️ Ошибка определения года: {e}")
        # Возвращаем папку по умолчанию (2025)
    ...
    for index, pdf_file in enumerate(pdf_files, 1):
        ...
        try:
            ...
            try:
                ...
                print(f"  ❌ Ошибка чтения PDF: {pdf_error}")
                success, new_name, _ = rename_and_move_file(pdf_path, MANUAL_FOLDER, 'manual_')
                if success:
                    print(f"  → manual/{new_name}")
                    log_processing(f"ERROR (чтение): {pdf_file} -> {new_name} - {pdf_error}")
                    manual_files.append(pdf_file)
                    if TELEGRAM_ENABLED and DETAILED_NOTIFICATIONS:
                        telegram.notify_manual(pdf_file, f"Ошибка чтения PDF: {str(pdf_error)[:80]}")
                error_count += 1
                continue
